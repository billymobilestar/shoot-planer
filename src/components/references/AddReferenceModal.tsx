...
import { X, Upload, Plus, Loader2, MapPin, Check, Link2, Image as ImageIcon, AlertCircle } from "lucide-react";
...
export default function AddReferenceModal({ projectId, locations, boards, onCreated, onClose }: Props) {
  ...
  function handleLinkUrlChange(value: string) {
    ...
    // Auto-fetch preview after a short delay once a valid URL is entered
    try {
      new URL(value.trim());
    } catch {
  ...
  async function fetchLinkPreview(url: string) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Auto-set thumbnail if available and none set
        if (data.image && !linkThumbnail) setLinkThumbnail(data.image);
        setPreviewFetched(true);
        if (!data.image) {
          setPreviewError("No preview image available for this link");
        }
      } else {
        setPreviewError("Preview could not be generated for this link");
        setPreviewFetched(true);
      }
    } catch {
      setPreviewError("Preview could not be generated for this link");
            ...
            <>
              <div>
                ...
                <input
                  ...
                  autoFocus
                />
                {linkUrl && detectPlatform(linkUrl) && !previewLoading && (
              ...
              {previewLoading && (
                <div className="flex items-center gap-2 py-3 px-4 bg-bg-primary border border-border rounded-lg">
                ...
                {linkThumbnail ? (
                  <div>
                    ...
                    <div className="relative w-full max-w-xs">
                      <img src={linkThumbnail} alt="" className="w-full rounded-lg object-cover max-h-40" />
                      <button type="button" onClick={() => setLinkThumbnail("")} className="absolute top-1.5 right-1.5 bg-black/60 rounded-full p-1 text-white hover:bg-black/80">
                        <X className="w-3.5 h-3.5" />
                      </button>
                    </div>
                  </div>
                ) : !previewLoading && previewFetched && previewError ? (
                  <div className="flex items-center gap-2 py-2.5 px-3 bg-warning/5 border border-warning/20 rounded-lg">
                    <AlertCircle className="w-4 h-4 text-warning shrink-0" />
                    <span className="text-xs text-warning">{previewError}</span>
                    <label className="ml-auto inline-flex items-center gap-1 text-xs text-accent hover:text-accent-hover cursor-pointer shrink-0">
                      <Upload className="w-3 h-3" />
                      Upload
                      <input type="file" accept="image/*" onChange={handleThumbUpload} className="hidden" />
                    </label>
                  </div>
                ) : !previewLoading && !linkThumbnail && !previewFetched && linkUrl ? (
